...
function handleGetNowClick(location: string) {
  ...
  if (typeof window !== 'undefined' && typeof (window as any).fbq === 'function') {
    ;(window as any).fbq('trackCustom', 'FreeConsultClick', { button_location: location, page_name: 'Expert_TY', cta: 'consultation' })
  }
  ...
  if (typeof window !== 'undefined' && typeof (window as any).gtag === 'function') {
    ;(window as any).gtag('event', 'get_now', { button_location: location, page_name: 'Expert_TY' })
  }
  // Scroll to the expert-ty-cta section
  const ctaEl = document.getElementById('expert-ty-cta')
  if (ctaEl) {
...
function FinalCTASection() {
  return (
    <section id="expert-ty-cta" className="py-16 lg:py-24 purple-gradient text-white relative overflow-hidden">
...
/* ──────────────────── MAIN PAGE COMPONENT ──────────────────── */
export default function ExpertTYPage() {
  useEffect(() => {
    // Meta Pixel: CompleteRegistration on page load
    if (typeof window !== 'undefined' && typeof (window as any).fbq === 'function') {
    ...
    if (typeof window !== 'undefined' && typeof (window as any).gtag === 'function') {
      ;(window as any).gtag('event', 'complete_registration', {
        event_category: 'conversion',
        event_label: 'Expert TY Page Load',
        page_name: 'Expert_TY',
      })
    }
  }, [])
